...
import { SEMEJANZAS, SEMEJANZAS_RECHAZADAS } from "@/e2e/fixtures/semejanza";

export const metadata: Metadata = {
  title: "Vista previa — semejanza",
  robots: { index: false, follow: false },
};

/**
 * Ruta de previsualización, no de producto: monta el bloque de visualización
 * de semejanza con datos de prueba (`e2e/fixtures/`), no con una lección real.
 * Mismo patrón que `/vista-previa/cuerpos-geometricos`. Ningún enlace de la
 * aplicación apunta acá.
 */
export default function VistaPreviaSemejanza() {
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      <p className="mb-3 text-sm text-ink-suave">
        ...
        su descripción, nunca reventar la página.
      </p>
      <div className="grid gap-5 sm:grid-cols-2">
        {SEMEJANZAS_RECHAZADAS.map(({ titulo, bloque }) => (
          <section key={titulo} data-caso={titulo}>
            <h3 className="mb-1.5 text-sm text-ink-suave">{titulo}</h3>
            <BloqueVisualizacion bloque={bloque} />
          </section>
