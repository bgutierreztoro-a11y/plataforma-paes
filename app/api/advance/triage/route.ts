...
import { validarCuerpoTriage } from "@/lib/advance/triage";
import { registrarSesionTriage } from "@/lib/datos/advanceTriage";
import { DatosError } from "@/lib/datos/db";

/**
 * POST /api/advance/triage: guarda una sesión de triage terminada
 * (docs/fobos-advance.md §6.5, F5a). Solo escribe; no devuelve nada.
 *
 * MISMO ORDEN NO NEGOCIABLE que POST /api/advance/sesion. Primero la flag,
 * después la sesión de Clerk, después el gate de producto, después el cuerpo,
 * y recién ahí la base. Sin sesión se responde 401 sin haber tocado Postgres.
 * `usuario_id` sale de auth() y nunca del cuerpo.
 *
 * El cuerpo no se confía: forma, tipos y alfabeto de `decision` en
 * lib/advance/triage.ts, y la unidad y sus ítems se cotejan contra el banco
 * en disco. El veredicto no viaja ni se guarda (D17).
 *
...
export async function POST(req: Request) {
  ...
  const cuerpo = validarCuerpoTriage(entrada);
  ...
  try {
    await registrarSesionTriage(userId, cuerpo.sesionId, cuerpo.unidadId, cuerpo.registros);
  } catch (e) {
    ...
    console.error(`[advance-triage] ${detalle}`);
