...
- **Por repasar:** superficie clara, anillo ámbar y **flecha circular de "revisar"** al centro. Es lo que se pinta cuando el estudiante terminó pero quedó bajo el umbral de dominio. Ámbar y **nunca rojo**, y flecha y **nunca equis**, por la misma razón que §3.4: terminar flojo es una invitación a volver, no una falla. **No bloquea nada** — la lección siguiente sigue disponible.
- **Completada:** disco relleno `--color-success` con check blanco.
- **En construcción:** relleno gris plano, contorno punteado, sin sombra y sin candado. Ver más abajo.

> **Enmienda del 2026-07-31 (glifo por estado).** Esta lista describía "en curso" y "por repasar" como **el mismo punto con anillo**, distinguidos solo por el tono —azul contra ámbar—. Es exactamente la distinción por color solo que §2.1 prohíbe y que el párrafo de arriba cita como regla. Ahora cada estado tiene glifo propio: check, flecha de revisar, punto lleno, punto blanco. Lo mismo le pasaba a la meta, que se distinguía **solo por tamaño y contorno**: un cierre disponible y una lección disponible dibujaban el mismo punto blanco, y la diferencia solo se leía si estaban una al lado de la otra, que en el recorrido nunca ocurre. La meta lleva ahora un banderín, y cede al check cuando está terminada. El disco de "en construcción" pasa de tener el color de fondo de la página —era un hueco en la cuadrícula, no un disco— a un gris plano, y su punteado sube de `--color-border-strong` (1,42:1, prácticamente invisible) a `--ink-400` (2,99:1): desde que esos nodos son seleccionables para explicar qué les falta, dejaron de ser un adorno. No se sube más a propósito — con 13 de 16 unidades en construcción, un gris de alto contraste le ganaría la atención a las que sí se pueden abrir.
