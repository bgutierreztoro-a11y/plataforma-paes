...
/**
 * Qué color termina siendo `--linea-nav` en una línea dada, leyendo el mapa real
 * en vez de repetirlo acá.
 *
 * Las dos formas que el mapa usa hoy: `var(--linea)` —que es el color de la
 * propia línea, ver `estiloDeLinea()`— y `var(--text-primary)`, la tinta a la
 * que baja la 02. Cualquier tercera forma cae en el `assert` de abajo en vez de
 * pasar en silencio.
 */
function siluetaDeLinea(id: string): [number, number, number] {
  const mapa = /const NAV_POR_LINEA[^{]*\{([^}]*)\}/.exec(COLORES);
  assert.ok(mapa, "NAV_POR_LINEA ya no está en components/ui/linea/colores.ts");
  const entrada = new RegExp(`"${id}":\\s*"var\\(--([a-z0-9-]+)\\)"`).exec(mapa[1]);
  assert.ok(entrada, `NAV_POR_LINEA ya no tiene entrada para la línea ${id}`);
...
describe("El sello de estación se ve en las cuatro líneas", () => {
  ...
  test("el núcleo del sello lleva el color del eje, no la silueta", () => {
    /* La identidad del eje vive en el relleno, que es masa dentro de un anillo
       que ya carga la legibilidad. Es lo que deja que la 02 siga siendo amarilla
       en la pantalla sin recalibrar `--line-02` ni `Estacion.tsx`.

       Lo que separa un estado del otro es la **presencia** del disco, no su
       contraste: un disco lleno dentro de un anillo de 5px se distingue de uno
       vacío aunque el relleno sea claro. Medido sobre el fondo de página, el
       relleno da 4,57 · 1,66 · 4,54 · 6,48 — la 02 es la más tenue de las cuatro
       y es la razón por la que el anillo, y no el relleno, sostiene la pieza. */
    ...
    assert.equal(nucleo[1], "linea", "El núcleo dejó de llevar el color del eje");
  });
});
