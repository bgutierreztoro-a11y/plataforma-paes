...
const CATALOGO = new Map<string, EntradaError>([
  ["falla-1", { id: "falla-1", descripcion: "Descripción del error uno.", titulo: "Título uno", apoyo: "Apoyo uno." }],
  ["falla-2", { id: "falla-2", descripcion: "Descripción del error dos.", titulo: "Título dos", apoyo: "Apoyo dos." }],
  ["falla-3", { id: "falla-3", descripcion: "Descripción del error tres." }],
]);
...
describe("tarjetasDeUnidad", () => {
  it("traduce las tres fases del dominio a los nombres visibles (D8)", () => {
    const tarjetas = tarjetasDeUnidad(
      UNIDAD,
      [estado("falla-1", "abierto"), estado("falla-2", "observacion"), estado("falla-3", "cerrado")],
      CATALOGO,
    );
    assert.deepEqual(
  ...
  it("sin-datos no genera tarjeta (D9); sin ningún intento, la lista queda vacía", () => {
    assert.deepEqual(tarjetasDeUnidad(UNIDAD, [estado("falla-1", "sin-datos")], CATALOGO), []);
  ...
  it("copia titulo y apoyo del catálogo, nunca id ni descripcion cuando hay titulo", () => {
    const [t] = tarjetasDeUnidad(UNIDAD, [estado("falla-2", "abierto")], CATALOGO);
  ...
  it("un error sin titulo en el catálogo cae a descripcion, sin apoyo (F4b, catálogos sin copy todavía)", () => {
    const [t] = tarjetasDeUnidad(UNIDAD, [estado("falla-3", "abierto")], CATALOGO);
  ...
  it("un error observado que no está en el catálogo se omite (D12)", () => {
    const tarjetas = tarjetasDeUnidad(
      UNIDAD,
      [estado("error-fantasma", "abierto"), estado("falla-1", "abierto")],
      CATALOGO,
    );
    assert.deepEqual(
  ...
  it("recaidas >= 1 marca la recaída; 0 no (D11)", () => {
    const tarjetas = tarjetasDeUnidad(
      UNIDAD,
      [estado("falla-1", "abierto", { recaidas: 1 }), estado("falla-2", "abierto", { recaidas: 0 })],
      CATALOGO,
    );
    assert.deepEqual(
  ...
  it("p(L) no está en el modelo de vista (D10)", () => {
    const [t] = tarjetasDeUnidad(UNIDAD, [estado("falla-1", "abierto", { pL: 0.42 })], CATALOGO);
    assert.ok(!("pL" in t));
  ...
  it("unidadId viaja en todas las tarjetas, el mismo que se pasó a tarjetasDeUnidad", () => {
    const tarjetas = tarjetasDeUnidad(
      UNIDAD,
      [estado("falla-1", "abierto"), estado("falla-2", "observacion")],
      CATALOGO,
    );
  ...
  it("ordena por fase (por repasar, en estudio, superado) y dentro de cada fase por último intento, más reciente arriba", () => {
    ...
    CATALOGO_4.set("falla-4", { id: "falla-4", descripcion: "Descripción del error cuatro.", titulo: "Título cuatro" });
    const tarjetas = tarjetasDeUnidad(
      UNIDAD,
      [
        estado("falla-3", "cerrado"),
        estado("falla-1", "abierto", { ultimoIntentoMs: T }),
        estado("falla-2", "observacion"),
        estado("falla-4", "abierto", { ultimoIntentoMs: T + 1 }),
      ],
      CATALOGO_4,
    );
    assert.deepEqual(
      tarjetas.map((t) => t.errorId),
      ["falla-4", "falla-1", "falla-2", "falla-3"],
    );
