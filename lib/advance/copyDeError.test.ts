...
/* Tres entradas, las mismas formas que en pantallaErrores.test.ts: copy
   ...
   titulo sin apoyo. */
const COMPLETA: EntradaError = { id: "error-1", descripcion: "Descripción del error uno.", titulo: "Título uno", apoyo: "Apoyo uno." };
const SIN_COPY: EntradaError = { id: "error-3", descripcion: "Descripción del error tres." };
const SIN_APOYO: EntradaError = { id: "error-4", descripcion: "Descripción del error cuatro.", titulo: "Título cuatro" };
...
describe("copyDelCatalogo", () => {
  it("conserva las claves del catálogo y proyecta cada entrada", () => {
    ...
    assert.deepEqual(copy["error-3"], { titulo: SIN_COPY.descripcion });
  });
