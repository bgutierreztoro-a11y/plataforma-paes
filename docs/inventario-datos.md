...
| PostHog | Eventos de producto declarados en `lib/eventos.ts` y en CLAUDE.md, con ids de lección, ítem, paso, unidad, error del catálogo, tiempos y contadores. Los del recorrido de entrada suman respuestas cerradas de la bienvenida y `video`, el id del video de origen validado por `lib/recorrido/video.ts`; hoy salen los tres de la página pública del error. PostHog agrega por su cuenta `$current_url` con la URL completa, así que en esa página también le llegan los parámetros `utm_*` del link tal como vienen. `lib/eventos.test.ts` hace fallar `tsc` si un evento acepta correo, nombre, RUT, colegio u otra clave personal. Sin autocapture, sin grabación de sesión, sin `identify()`: cada carga de página es un visitante anónimo nuevo | Medir aprendizaje y uso (MOS §8) | La del proyecto de PostHog. Sin identidad, no hay nada que borrar a pedido de un titular |
