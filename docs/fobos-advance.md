...
- (18) Forma: `orientacion` en el vocabulario; `eje` con `min`, `max` y `paso` finitos, `etiqueta` texto y `grilla` booleano si vienen; 1 a 5 cajas (2024 regular n.º 61 trae 5); los cinco números finitos; `rotulos` booleano; `nombre` de hasta 14 caracteres; `descripcion` de 30 o más; sin claves sobrantes.
...
- (22) Marcas: a lo más 11 (el tope del histograma, 10 intervalos y 11 bordes con número, la otra figura de datos con un eje numérico rotulado a lo ancho).
- (23) Nombres: obligatorios y únicos con 2 o más cajas; con una, opcional. En vertical el nombre va centrado bajo su caja y tiene que caber en su carril.
...
- (27) Marcas según el número más largo: no más marcas de las que caben en el eje sin que sus números se pisen con letra de 12 px. En horizontal cada marca ocupa el ancho del número más largo del eje más el aire; en vertical, una letra de alto más el aire. Con números de 4 cifras ("1.000", 5 caracteres) caben 9 en el enunciado y 8 en una alternativa; en vertical caben las 11. Medido en el render real: 11 marcas horizontales de 4 cifras pisan los diez pares de números vecinos en las dos ubicaciones.

Las reglas (23), (24) y (27) miden con la misma geometría que dibuja el componente (`lib/advance/diagramaCajon.ts`: `geometriaCajon`, `choquesDeRotulos`, `nombresQueNoCaben`, `limiteDeMarcas`), en la ubicación de la figura: la del ítem se mide como enunciado y la de una alternativa como alternativa. El SVG se escala entero, así que un choque medido en unidades del viewBox es un choque a 390 px. El ancho de un número se estima en 0,6 em por cifra y 0,3 em por coma o punto, un poco por encima de lo real, así que el aviso llega antes que el choque.

Dibujo (`components/advance/figuras/DiagramaCajon.tsx`): la caja va en `--linea-tinte` con borde `--linea-nav` de 1,5; la mediana en tinta con trazo 3, así que se distingue por grosor y no por color; los bigotes llegan al mínimo y al máximo con remate perpendicular. En horizontal: nombre a la izquierda, q1, mediana y q3 sobre la caja, mínimo y máximo bajo los bigotes, eje abajo y unidad bajo su extremo derecho. En vertical: eje a la izquierda con la unidad arriba, nombre bajo cada carril y los rótulos a la derecha de la caja. La mediana rotulada va en negrita. Con cinco carriles verticales la caja se adelgaza (hasta 6 unidades de medio ancho) para dejar lugar a los rótulos.

Letra (firmado 2026-09-22): ningún texto del cajón baja de 12 px a 390 × 844, ni números del eje, ni rótulos, ni nombres, ni unidad. Todo texto va en 12 unidades del viewBox, y el viewBox de cada ubicación es igual o menor que su carril más angosto, medido en la galería: 320 en el enunciado (carril de 324 px; en descarte y triage reales, 358) y 280 en una alternativa (carril de 282 px en triage de la galería; en las rutas reales, 316). El componente recibe la ubicación como `contexto`; `FiguraDeAlternativa` lo monta con `"alternativa"`. Ancho máximo de 26rem en el enunciado y 22rem en una alternativa, para que en escritorio no crezca sin límite. Sin ResizeObserver ni medición en cliente: el SVG escala con CSS y el render es el mismo en servidor y cliente. Accesibilidad como `PlanoFuncion`: `role="img"`, `<title>` generado desde `textos.ts` ("Diagrama de cajón", "Cinco diagramas de cajón") y `<desc>` = `descripcion`. `figuraParaCliente` pasa `nombre` y `eje.etiqueta` por `protegerExpresiones` y deja los números intactos.

Galería `/_design`, sección "Diagrama de cajón", en la línea 04 con datos inventados (`CAJONES_MUESTRA`, `CAJONES_ALTERNATIVA_MUESTRA`): una caja horizontal rotulada, q1 igual a la mediana, tres verticales leídas del eje, dos verticales rotuladas, cuatro horizontales con valores negativos, cinco horizontales y cinco verticales rotuladas, y el peor caso de marcas de 4 cifras en enunciado y en alternativa (11 en vertical; en horizontal, el máximo que admite la regla 27). Medido a 390 × 844 en claro y oscuro (el repo no tiene tema oscuro y las capturas salen idénticas): 0 desbordes, 0 textos pisados y letra mínima de 12,15 px en el enunciado y 13,54 px en la alternativa.
