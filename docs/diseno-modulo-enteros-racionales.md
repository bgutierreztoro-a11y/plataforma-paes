...
|---|---|---|
| pierde-signo-al-restar-negativo | Al restar un negativo, invertir el signo del resultado (trata `a − (−b)` como `a − b`, perdiendo el cambio de signo). | Distinto de `lineal-patrones-de-cambio` confunde-direccion-al-despejar/confunde-signo-de-la-tasa (esos son sobre despejar valores desconocidos o confundir el signo de una tasa de cambio hacia adelante, no sobre la mecánica de restar un negativo). |
| invierte-orden-de-la-diferencia | Invertir el orden de la diferencia entre dos profundidades/valores (calcula B − A cuando corresponde A − B, o viceversa), sin error de signo en sí — es un error de qué se resta de qué. | Distinto de pierde-signo-al-restar-negativo: aquí el signo de cada término está bien tomado, lo que falla es el orden de la resta. |
| trata-negativo-como-distancia | Tratar el valor negativo como si fuera una distancia (magnitud) y perder el signo al usarlo en una operación posterior, aunque la resta inicial haya estado bien planteada. | No se solapa con errores de `ecuaciones-lineales` (esos son sobre equilibrio de la balanza, no sobre naturaleza del signo). |
| compara-fracciones-solo-por-numerador | Comparar dos fracciones fijándose solo en el numerador (cree que 3/4 > 2/3 porque 3 > 2, ignorando el denominador). | Error clásico de racionales, no cubierto en ningún catálogo existente del proyecto. |
| compara-negativos-como-positivos | Comparar dos números negativos como si fueran positivos (cree que −3/4 < −4/5 porque 3/4 < 4/5 en valor absoluto). | Específico de negativos + fracciones combinados; distinto de compara-fracciones-solo-por-numerador, que es sobre fracciones positivas. |
| suma-denominadores | Sumar denominadores al sumar o restar fracciones (calcula a/b + c/d como (a+c)/(b+d)). | Error clásico de racionales, no cubierto en ningún catálogo existente. |
| rechaza-division-que-agranda | Creer que dividir siempre achica (al ver 3 ÷ 3/4 = 4, rechaza el resultado o busca dónde "se perdió" un paso porque el resultado es mayor que 3). | Es el error que el insight del paso 5 de L2 está diseñado para desarmar directamente. |
